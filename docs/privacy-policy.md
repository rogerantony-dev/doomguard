...
Unhook is a screen-time app for Android. It counts the time you spend on
...
This policy describes what Unhook does with your information. The short version:
Unhook does not collect it, does not send it anywhere, and does not have a server
to send it to.

## What Unhook stores

Unhook stores the following on your device only:
...
## What Unhook does not do

- **No accounts.** There is no sign-up, no login, and no profile.
- **No servers.** Unhook contains no code that sends data over the internet. It
...
Unhook uses Android's Accessibility Service API. This is the only way an Android
app can tell what is currently on screen in another app, and Unhook needs that to
...
- **Internet and network state.** These appear in the app's permission list
  because standard Android app frameworks declare them. Unhook itself makes no
  network requests.
- **Vibrate.** Declared by the app framework.

## Children

Unhook is not directed at children and does not knowingly collect any
information from anyone, regardless of age.

## Removing your data

Uninstalling Unhook deletes everything it has stored. Because nothing is held
...
the date at the top will change. If a future version of Unhook ever collects or
